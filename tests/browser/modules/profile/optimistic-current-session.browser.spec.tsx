...
} from '@/modules/profile/presentation/optimistic-current-session';
...
test('shared session observers roll back and reconcile optimistic names', async () => {
  ...
  await expect
    .element(page.getByTestId('profile'))
    .toHaveTextContent('Optimistic');
  ...
  await expect.element(page.getByTestId('profile')).toHaveTextContent('Before');
